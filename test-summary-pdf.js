// Local test for node10-summary-pdf
const fs = require('fs');
...
const $json = {
  choices: [{
    message: {
      role: 'assistant',
      content: `## Executive Summary\n- **KI-Gigafabriken:** Deutsche Bewerber kritisieren reduzierte Investitionen, doch die tatsächlichen Zahlen sind unklar.\n- **Digitalministerium-Umzug:** Das BMDS beginnt mit dem Umzug in neue Dienststäbe in Berlin.\n- **US-Telekommunikationsaufsicht:** Ausnahmegenehmigungen für ausländische Router werden erweitert.\n- **EU-US-Zusammenarbeit:** Die EU plant, die US-Regierung bei Verfahren gegen US-Digitalkonzerne einzubeziehen.\n- **KI-generierte Inhalte:** Die EU verbietet KI-generierte Bilder und Videos in offizieller Kommunikation.\n- **Medienkompetenz:** Im CDU/CSU-regulierten Bundesländern wird Medienkompetenz verstärkt gefördert.`
    }
  }]
};
...
fs.writeFileSync('/tmp/test-summary.pdf', buf);
console.log(`PDF written: ${buf.length} bytes, ${N} page(s)`);
console.log('Open: open /tmp/test-summary.pdf');
