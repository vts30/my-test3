...
# Note: intentionally NOT using set -e so terraform exit code 2 (changes present)
...
REPORT_FILE="${SCRIPT_DIR}/tf-plan-report-$(date +%Y%m%d-%H%M%S).txt"
CONTEXT_SUFFIX="preme-fqdn"
...
switch_context() {
  ...
  if ! kubectl config get-contexts "$ctx" &>/dev/null; then
    echo -e "${YELLOW}  Warning:${RESET} kube context '$ctx' not found — skipping." >&2
    return 0
  fi
...
# Returns: 0 = no changes, 2 = has changes, 1 = error
run_plan() {
  local stage_dir="$1" out_file="$2" env="$3" region="$4"
  ...
  terraform plan -input=false -no-color -out="${out_file}.tfplan" >> "$log_file" 2>&1
  local exit_code=$?
...
# ── List changed resources (same as: terraform plan | grep "# module\.") ──────
parse_changes() {
  grep "# module\." "$1" 2>/dev/null | sed 's/^\s*/  /' | head -50
}
...
# ── Step 1: run all plans ─────────────────────────────────────────────────────
for stage in "${STAGES[@]}"; do
  name=$(basename "$stage")
  env=$(echo "$name"    | grep -oE '(idst|dst)')
  region=$(echo "$name" | grep -oE 'vfz[0-9]+')
  out="${TMP_DIR}/${name}.plan"

  echo -e "  ${CYAN}planning${RESET}  $name ..."
  run_plan "$stage" "$out" "$env" "$region" || true
  PLAN_FILES[$stage]="$out"
done

# ── Step 2: calculate totals in main shell (not a subshell) ──────────────────
total_add=0; total_chg=0; total_destroy=0
error_count=0; no_change_count=0; change_count=0
...
# ── Step 3: write report directly to file, then display it ───────────────────
{
  echo "==============================================================="
  ...
  for stage in "${STAGES[@]}"; do
    ...
    if [[ "$summary" == "ERROR" ]]; then
      echo "[ ERROR ] $name  (env: $env, region: $region)"
    ...
    if [[ $add -eq 0 && $chg -eq 0 && $destroy -eq 0 ]]; then
      echo "[ NO CHANGES ] $name  (env: $env, region: $region)"
    else
...
echo ""
[[ $error_count   -gt 0 ]] && echo -e "${RED}${BOLD}$error_count stage(s) failed to plan — check the .log files.${RESET}"
[[ $total_destroy -gt 0 ]] && echo -e "${RED}${BOLD}WARNING: $total_destroy resource(s) will be destroyed!${RESET}"
[[ $total_chg     -gt 0 ]] && echo -e "${YELLOW}$total_chg resource(s) will be changed.${RESET}"
[[ $change_count -eq 0 && $error_count -eq 0 ]] && echo -e "${GREEN}All stages are up to date — no changes needed.${RESET}"
